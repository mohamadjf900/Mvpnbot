...
# ========== توابع پروکسی (اصلاح‌شده) ==========
async def add_proxy(proxy_type: str, ip: str, port: int, url: str = None, remarks: str = None):
    """افزودن پروکسی با پشتیبانی از لینک مستقیم"""
    async with aiosqlite.connect(DB_PATH) as db:
        if url:
            # لینک مستقیم (مثل https://t.me/proxy?...)
            await db.execute(
            ...
            await db.execute(
                ...
                (proxy_type.upper(), ip, port)
            )
        await db.commit()

async def get_active_proxies(limit: int = 50):
    """دریافت پروکسی‌های فعال (اولویت با لینک‌های مستقیم)"""
    async with aiosqlite.connect(DB_PATH) as db:
        # اول پروکسی‌های دارای لینک مستقیم
        async with db.execute(
            "SELECT url, remarks FROM proxies WHERE is_active = 1 AND url IS NOT NULL ORDER BY RANDOM() LIMIT ?",
            (limit,)
        ) as cursor:
            rows = await cursor.fetchall()
            results = [{"url": row[0], "remarks": row[1]} for row in rows]
        
        # اگر تعداد کافی نبود، پروکسی‌های معمولی را اضافه کن
        if len(results) < limit:
            ...
            async with db.execute(
                ...
                for row in rows2:
                    results.append({"type": row[0], "ip": row[1], "port": row[2]})
        return results
